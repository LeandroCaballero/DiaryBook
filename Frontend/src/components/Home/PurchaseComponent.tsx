...
const PurchaseItem = ({ item, navigation }: ChildProps) => {
  ...
  return (
    <TouchableOpacity
      ...
      onPress={() => navigation.navigate("PurchaseDetails", { purchase: item })}
    >
      <View className="flex flex-row gap-x-2 items-center">
        <View className="w-fit border-2 p-1 rounded-md">
          <ShoppingCartIcon size={30} color="#000000" />
        </View>
        <Text className="w-3/6 col-span-5 ">{item.name}</Text>
        <View className="flex flex-col w-2/6">
          <Text className="">{calculateTotalPurchaseItem(item)}</Text>
          <Text className="">{dayjs(item.dateBuy).format("DD [de] MMMM")}</Text>
        </View>
      </View>
    </TouchableOpacity>
  )
